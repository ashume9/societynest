/*
  ...
  1. New Data
    - Creates a test partner with username 'ramu_partner'
    - Sets up partner with 'both' type (ironing and delivery)
    - Creates a partner session for easy testing

  2. Implementation Details
    - Uses the first available society and tower
    - Sets up proper working days and holiday schedule
    - Creates a session token for immediate testing
*/
...
BEGIN
  ...
  -- Get the first tower in that society (using table alias to avoid ambiguity)
  SELECT t.id INTO first_tower_id 
  FROM towers t 
  WHERE t.society_id = first_society_id 
