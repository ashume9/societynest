-- Create sample order data for testing partner dashboard functionality
-- This migration creates orders for user 'ashu' with various statuses
...
DECLARE
    ...
    kurta_id uuid;
BEGIN
    -- Get existing user "ashu" with proper column qualification
    ...
    -- If ramu partner doesn't exist, throw error
    IF partner_id IS NULL THEN
        RAISE EXCEPTION 'Partner with username "ramu" not found. Please ensure the partner exists before running this migration.';
    END IF;
    ...
    RAISE NOTICE 'Partner: ramu (ID: %)', partner_id;
    RAISE NOTICE 'Created 5 orders with statuses: pending, picked_up, in_progress, ready, delivered';
    RAISE NOTICE 'Order 1 (PENDING): 3 shirts + 2 trousers = ₹85';
    RAISE NOTICE 'Order 2 (PICKED_UP): 2 shirts + 1 saree + 1 kurta = ₹130';
    RAISE NOTICE 'Order 3 (IN_PROGRESS): 4 shirts + 3 trousers = ₹190 (assigned to ramu)';
    RAISE NOTICE 'Order 4 (READY): 1 saree + 3 kurtas = ₹95 (completed by ramu)';
    RAISE NOTICE 'Order 5 (DELIVERED): 2 shirts + 2 sarees = ₹160 (delivered by ramu)';
