/*
  ...
  1. Database Changes
    ...
    - Update RLS policies for proper user creation
    - Fix username constraint to allow nulls
    - Make username and phone nullable during signup

  2. Security
    - Enable RLS policies for authenticated users
    - Add service role policy for trigger function
...
-- Fix username constraint by dropping the constraint and recreating as partial index
ALTER TABLE users DROP CONSTRAINT IF EXISTS users_username_key;
CREATE UNIQUE INDEX IF NOT EXISTS users_username_key ON users(username) WHERE username IS NOT NULL;

-- Make username and phone nullable temporarily for auto-creation
