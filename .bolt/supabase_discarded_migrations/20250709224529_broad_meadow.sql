/*
  ...
  1. New Test Orders
    - Add orders in various states for delivery partners
    - Create orders for pickup, delivery, and completed states
    - Add realistic order items and amounts

  2. Test Data
    - Pending pickup orders
    - Ready for delivery orders
    - Assigned pickup/delivery orders
    - Completed pickup/delivery orders
*/
...
BEGIN
  ...
  -- Get first tower
  SELECT id INTO tower_id FROM towers WHERE society_id = society_id LIMIT 1;
  
  -- Get first flat
  SELECT id INTO flat_id FROM flats WHERE tower_id = tower_id LIMIT 1;
